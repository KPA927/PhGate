...
def expect(A, state):
    ...
    return np.abs(state.getH()@A@state)
...
class VirtualQubitSystem:
    ...
    def sesolve(self, calc_Phi, progress_bar = True, e_ops = []):
    ...
    def mesolve(self, calc_Phi, rho_flag = False, progress_bar = True):
