...
class VirtQ:
    ...
    def solveSE(self, calc_H_as_time_function, progress_bar = True, solver='expm', e_ops = None, savestep =  1):
        """
        ...
        :param e_ops: expectation values for given operators. Should be a list of functions like f(state). 
                                                    If None, all psi function will be saved.
        :param progress_bar(bool): show progress_bar
        :param solver: exmp or RK4
        :param savestep - the frequency the psi will be saved. If not given, the savestep = 1 which means that every step is saved. 
        :return: the array of size (Nsteps/savestep, size(e_ops))
        """
        if solver == 'RK4':
        ...
        for i in i_range:
            ...
            if np.sum(np.abs(np.asarray(tf.math.conj(psi)*psi))) >2:
                print("The step is too big, try to make it smaller. Stop iteration:", i)
                break
            if i%savestep==0:
                if e_ops == None:
                    psilist.append(psi)
    ...
    def solveME(self, calc_H_as_time_function, progress_bar = True,  e_ops = None, savestep =  1):
        self.calc_H_as_time_function = calc_H_as_time_function
